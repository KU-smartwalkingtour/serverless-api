...
const { deleteRecentCourse } = require('/opt/nodejs/services/userService');
...
exports.handler = async (event) => {
  ...
  logger.info('User delete recent course request', { userId, courseId });

  try {
    const result = await deleteRecentCourse(userId, courseId);
    return success(result);
  } catch (err) {
    logger.error('User delete recent course error', { error: err.message });
